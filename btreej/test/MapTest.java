...
public abstract class MapTest {
	...
	public void simpleRemove() {
	    reset();
	    populate(data.length / 2);
	    testMap.remove("Florida");
	    for (int i = 0; i < data.length; i += 2) 
	        if (! data[i].equals("Florida"))
	            assertTrue("Missing key: " + data[i], testMap.containsKey(data[i]));
	...
	public void allRemove() {
        ...
        for (int i = 0; i < data.length; i += 2) {
            String key = data[i];
            assertTrue("Missing key: " + key, testMap.containsKey(key));
            testMap.remove(key);
            assertFalse("Marvin K Mooney key: " + key, testMap.containsKey(key));
        }
